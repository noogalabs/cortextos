...
```bash
ascendops bus update-heartbeat "<1-sentence summary of current work>"
```
...
```bash
ascendops bus check-inbox
```

For any messages returned: process and ACK each one:

```bash
ascendops bus ack-inbox "<message_id>"
```
...
```bash
ascendops bus list-tasks --agent $CTX_AGENT_NAME --status pending
ascendops bus list-tasks --agent $CTX_AGENT_NAME --status in_progress
```
...
```bash
ascendops bus log-event heartbeat agent_heartbeat info --meta '{"agent":"'$CTX_AGENT_NAME'"}'
```
...
When done:
```bash
ascendops bus complete-task "<task_id>" --result "<summary of what was produced>"
```
