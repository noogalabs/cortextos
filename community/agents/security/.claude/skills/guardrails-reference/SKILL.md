...
| Creating a one-shot reminder or persistent cron | "CronCreate is enough, it'll persist" | CronCreate is session-only. Use `cortextos bus create-reminder` for one-shot reminders and `cortextos bus add-cron` for daemon-managed persistent crons. |
