...
3. **Use the permanent pinned opensrc install.** Set `OPENSRC_HOME=${CTX_ROOT}/state/codie/opensrc-cache` and use `${CTX_ROOT}/state/codie/opensrc-install/node_modules/.bin/opensrc`. The installed package must remain `opensrc 0.7.2`.
...
1. **Fetch GitHub Trending.**
   ```bash
   SCOUT_STATE="${CTX_ROOT}/state/codie"
...
3. **Cheap relevance filter.**
   ...
   ```bash
   SEEN_STATE="${CTX_ROOT}/state/codie/trending-scout-seen.json"
   CLASSIFIER=""
   for candidate in \
...
4. **Dedup.**
   Read `${CTX_ROOT}/state/codie/trending-scout-seen.json` if present. Skip repos studied in the last 14 days unless the repo looks especially relevant. Update this file after the digest is sent.
...
5. **Deep-study at most three.**
   ...
   export OPENSRC_HOME="${CTX_ROOT}/state/codie/opensrc-cache"
   OPENSRC_BIN="${CTX_ROOT}/state/codie/opensrc-install/node_modules/.bin/opensrc"
...
6. **Write and send the digest.**
   Write a local report under `${CTX_ROOT}/state/codie/trending-repo-scout/YYYY-MM-DD.md`, then send one bus message:
   ```bash
...
7. **Log and close the cron loop.**
   ```bash
   cortextos bus log-event action research_completed info --meta '{"agent":"codie","workflow":"trending-repo-scout"}'
